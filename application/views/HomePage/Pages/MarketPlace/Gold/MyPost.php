...
<div id="dialog" style='display:none; text-align:center; overflow:hidden;' >
	...
	<h3 class="sub-title">
        ...
        <div class="sub-title-btn">
		    <a id="btnPostGold" style="cursor:default"><img src="<?=base_url()?>Images/Button/post_btn.gif" alt="POST"></a>
	...
	<!-- <div class="ranking-search-wrap">
		...
		<fieldset class="ranking-search">
			<form method='POST' action="<?=base_url()?>homepanel/marketfilter" >
            ...
            <select id="selSearchFlag" name="selAmount" style="width:200px;">
				...
				<div class="right">
					...
					<img src="<?=base_url()?>Images/Button/search_btn.gif" id="btnSearch" class="pointer" alt="Find">
					<img src="<?=base_url()?>Images/Button/reset_btn.gif" id="btnReset" class="pointer" alt="Reset">
				</div>
				<div class="clear"></div>
				</form>
		</fieldset>
    ...
    $a = $this->Internal_model->GetChaNum($row['ChaNum']); 
    ...
    <tr>
    <td class="name" style="padding: 10px 0 8px 0; border-bottom: 1px solid #151515;"><center>'.$a.'</center></td>';
    ...
    <center>
    '.(($row['Stat'] == 1) ? "".(($this->session->userdata('UserID') != $UserNum) ? "
    ...
    <script>
        $('#btnPostGold').click(function() {
            if (IsAuth == 'False') {
                alert('Please try again after logging in..');
                location.href = '<?=base_url()?>Login';
                return false;
            }
            Common.OpenCenterWindow(500, 412, '<?=base_url()?>GoldMarket/SaleGold', 'GoldMarket', false);
        });
    </script>